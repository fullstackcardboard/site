const ActionsDisplayComponent = function() {
    ...
    for (let index = 0; index < keys.length; index++) {
      ...
      if (actions[key].triggers.length > 0) {
        for (let index = 0; index < actions[key].triggers.length; index++) {
          ...
          triggerNumbers += `<img src='content/images/trigger_${number}.png' class="rounded-circle" style="height: 5vh;" />`;
        }
      }

      html += `<div
            class="col-xs-12 col-sm-6 border shadow rounded bg-dark text-white action text-center mb-1"
            data-action="${key}"
            style="height: 8vh;"
          >
  ...
  function updatePossibleDisplay(actions) {
    ...
    for (let index = 0; index < keys.length; index++) {
      ...
      if (actions[key].triggers.length > 0) {
        ...
        for (let index = 0; index < actions[key].triggers.length; index++) {
          const number = actions[key].triggers[index];
          triggerNumbers += `<img src='content/images/trigger_${number}.png' class="rounded-circle mr-1" style="height: 5vh;" />`;
        }
        html += `
            <div
  ...
  function getImageHtml(key) {
    switch (key) {
      case "lab":
        return `
        <img src="${baseImageUrl}construct.png" style="height: 5vh;" />
        <img src="${baseImageUrl}lab.png" style="height: 5vh;" />
      `;
      case "power":
        return `
        <img src="${baseImageUrl}construct.png" style="height: 5vh;" />
        <img src="${baseImageUrl}power.png" style="height: 5vh;" />
      `;
      case "support":
        return `
        <img src="${baseImageUrl}construct.png" style="height: 5vh;" />
        <img src="${baseImageUrl}support.png" style="height: 5vh;" />
      `;
      case "factory":
        return `
        <img src="${baseImageUrl}construct.png" style="height: 5vh;" />
        <img src="${baseImageUrl}factory.png" style="height: 5vh;" />
      `;
      case "super":
        return `
        <img src="${baseImageUrl}construct.png" style="height: 5vh;" />
        <img src="${baseImageUrl}super.png" style="height: 5vh;" />
      `;
      case "supply":
        return `
        <img src="${baseImageUrl}supply.png" style="height: 5vh;" />
        <img src="${baseImageUrl}recruit.png" style="height: 5vh;" />
      `;
      default:
      case "power":
        return `
        <img src="${baseImageUrl}${key}.png" style="height: 5vh;" />
      `;
    }
  }
